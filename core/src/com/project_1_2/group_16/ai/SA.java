...
//                        System.out.println("Fitness: " + state.getFitness() + " vx: " + state.getVx() + " vy: " + state.getVy());
    ...
    private static boolean viableVector(StateVector sv){
        if((float)Math.sqrt(sv.velocity_x*sv.velocity_x+sv.velocity_y+sv.velocity_y) <= MAXVEL){
            return true;
        }
        return  false;
    }
    ...
    private Neighbour getRandomNeighbour(){
        float[] vxvy = validVelocity();
        return new Neighbour(new StateVector(hole_x, hole_y, vxvy[0], vxvy[1]));
    }
    ...
    private Neighbour getNeighbour(Neighbour state) {
       if(this.recalculate){
           ...
           for(int i = 0; i < newVectors.size(); i++){
               if(newVectors.get(i).velocity_x >= 0.01f && newVectors.get(i).velocity_y >= 0.01f && viableVector(newVectors.get(i))){
